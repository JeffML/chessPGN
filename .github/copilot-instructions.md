...
2. **Multi-Game PGN Parsing** - `indexPgnGames()` + `Cursor` for efficient iteration over large PGN files

The library maintains backward compatibility with chess.js while adding enhanced PGN parsing capabilities with worker thread support.
...
5. **Parity tests** - Adding Game methods requires ChessPGN wrapper + parity test
